...
public class MainScreenControllerOld extends ScreenController {
    ...
    public MainScreenControllerOld() {
    ...
    private void initialize() {
        tileSize = canvas.getHeight() / Main.simulator.getCurrentFloor().getRows();
    ...
    private void play() {
        ...
        new Thread(() -> {
            while (true) {
                ...
                for (Patch start : Main.simulator.getCurrentFloor().getStarts()) {
                ...
                for (Passenger passenger : Main.simulator.getCurrentFloor().getPassengers()) {
                    ...
                    if (Main.simulator.getCurrentFloor().checkGoal(passenger)) {
                        // Check if the goal the passenger is on allows this passenger to pass
                        if (Main.simulator.getCurrentFloor().checkPass(passenger, trainDoorsOpenButton.isSelected())) {
...
//                        Main.WALKWAY.removePassenger(passenger);
    ...
    private void generatePassenger(Patch start) {
        ...
        // TODO: The number of goals a passenger has depends on the passenger's type (SV vs SJ)
        Passenger passenger = new Passenger(
                start.getPatchCenterCoordinates().getX(),
                start.getPatchCenterCoordinates().getY(),
                Main.simulator.getCurrentFloor().getNumGoals()
        );
    ...
    private void drawListeners(String[] items, double tileSize) {
        Platform.runLater(() -> {
            ...
            Rectangle[][] rectangles = new Rectangle[Main.simulator.getCurrentFloor().getRows()][Main.simulator.getCurrentFloor().getColumns()];
            ...
            for (int row = 0; row < Main.simulator.getCurrentFloor().getRows(); row++) {
                for (int column = 0; column < Main.simulator.getCurrentFloor().getColumns(); column++) {
                    ...
                    rectangle.addEventFilter(MouseEvent.MOUSE_CLICKED, e -> {
                        ...
                        switch (drawState) {
                            case SPAWN:
                                Main.simulator.getCurrentFloor().setType(patchRow, patchColumn, Patch.Type.SPAWN, this.sequence);
                            ...
                            case CHECKPOINT:
                                if (isValidDraw) {
                                    ...
                                    Main.simulator.getCurrentFloor().setType(
                                            patchRow,
                                            patchColumn,
                                ...
                                break;
                            case DESPAWN:
                                Main.simulator.getCurrentFloor().setType(patchRow, patchColumn, Patch.Type.DESPAWN, this.sequence);

                                break;
                            case OBSTACLE:
                                Main.simulator.getCurrentFloor().setType(patchRow, patchColumn, Patch.Type.OBSTACLE, this.sequence);
                            ...
                            case FLOOR_FIELDS:
                                ...
                                Patch goal = Main.simulator.getCurrentFloor().getGoalFromGoalId(
                                        targetChoiceBox.getSelectionModel().getSelectedItem()
                                );

                                Main.simulator.getCurrentFloor().setType(patchRow, patchColumn, Patch.Type.CLEAR, this.sequence);

                                assert state != null;

                                // Make sure that there will only be one apex patch
                                Main.simulator.getCurrentFloor().setFloorField(
                                        patch,
                                        state,
                                        goal,
                            ...
                            if (state == PassengerMovement.State.IN_QUEUE) {
                                ...
                                if (floorField == 0.0) {
                                    ...
                                    color = Color.hsb(120.0,
                                            floorField / Main.simulator.getCurrentFloor().getMaximumFloorFieldValue(state),
                                            1.0
                                    );
                                }
...
//                graphicsContext.setFill(passenger.getColor());
            ...
            if (!hasStarted) {
                ...
                if (modeIndex == 0) {
                    ...
                    nextButton.setDisable(Main.simulator.getCurrentFloor().getStarts().size() == 0);
                    ...
                    if (sequence == Main.simulator.getCurrentFloor().getGoals().size()) {
                        stepButton.setDisable(true);
                    ...
                    nextButton.setDisable(sequence == Main.simulator.getCurrentFloor().getGoals().size());
                    stepButton.setDisable(true);
                    ...
                    if (!isDrawingFloorFields) {
                        ...
                        List<Patch> flattenedGoals = Main.simulator.getCurrentFloor().getGoalsFlattened();
            ...
            if (!Main.simulator.getCurrentFloor().getGoalsFlattened().isEmpty()) {
                for (Patch goal : Main.simulator.getCurrentFloor().getGoalsFlattened()) {
